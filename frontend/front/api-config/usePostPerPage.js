 ...
 */
 const usePostPerPage = (path) => {
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(false)
  const [statusCode, setStatusCode] = useState(0)

  const fetch = async (object, _page, _size) => {
    setLoading(true)
    setStatusCode(0)
    try {
      const { data: response, status } = await Axios.post(`${path}?page=${_page}&size=${_size}`, object, { validateStatus: false })
      setData(response)
